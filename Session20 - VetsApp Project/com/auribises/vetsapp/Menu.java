...
public class Menu {
	...
	private void showPetMenu() {
		...
		while(true) {
			...
			if(choice == 1) {
				scanner.nextLine();
				System.out.println("Enter PetOwner Phone Number: ");
				String phone = scanner.nextLine();
				...
				pet.readData();
				// Copy Operation -> Owner's id into Pet Owner's Id
				pet.oid = owner.oid;
				
				sql = pet.getInsertSql();
				int result = db.execute(sql);
				if(result > 0) {
					System.out.println("Pet "+pet.name+" Added...");
				}else {
				...
				// Fetch Pets of the same Owner
				sql = pet.getSelectSql(owner.oid, 0);
				db.fetchPets(sql);
				
				System.out.println("Enter the Pet ID to Update: ");
				pet.pid = scanner.nextInt();
				
				sql = pet.getSelectSql(0, pet.pid);
				pet = db.fetchPets(sql);
				...
				String age = scanner.nextLine();
				if(age.length() != 0) {
					pet.age = Integer.parseInt(age);
				}
				
				sql = pet.getUpdateSql();
				int result = db.execute(sql);
				if(result>0) {
					System.out.println("Update Successful for Pet ID:"+pet.oid);
				}else {
					System.out.println("Update Operation Failed..");
				}
				...
				if(deleteChoice.equalsIgnoreCase("yes")) {
					db.execute(sql);
					System.out.println(pet.pid+" deleted...");
				}
				
			}else if(choice == 4) {
				String sql = pet.getSelectSql(0, 0);
				db.fetchPets(sql);
			}else if(choice == 5) {
	...
	private void showConsultationMenu() {
		...
		while(true) {
			...
			System.out.println("3: View All Consultation");
			System.out.println("4: View Consultation Of a Pet");
			System.out.println("0: Quit");
			System.out.println();
			...
			if(choice == 1) {
				...
				System.out.println("Enter the Pet ID to Consult: ");
				pet.pid = scanner.nextInt();
				...
				consultation.readData();
				consultation.oid = owner.oid;
				consultation.pid = pet.pid;
				
				sql = consultation.getInsertSql();
				int result = db.execute(sql);
				if(result > 0) {
					System.out.println("Consultation for "+pet.name+" addedd...");
				}else {
					System.out.println("Consultation Failed..");
				}
				
			}else if(choice == 2) {
				
			}else if(choice == 3) {
				String sql = consultation.getSelectSql(0);
				db.fetchConsultations(sql);
			}else if(choice == 4) {
				// Fetch Pet Owner
				scanner.nextLine();
				System.out.println("Enter Phone Number: ");
				...
				sql = consultation.getSelectSql(pet.pid);
				db.fetchConsultations(sql);
